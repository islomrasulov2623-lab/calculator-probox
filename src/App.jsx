...
export default function SomInstallmentCalculator() {
  ...
  if (limitMode && months) {
    // --- Scoring limiti bo'yicha (YANGI MANTIQ) ---
    // Limit: bo'lib-bo'lib JAMI uchun chegara (oylik emas).
    // Ustama faqat qolgan summa (P − D) ga qo'llanadi.
    // Formulalar:
    //  requiredDown = max(0, P − L / (1 + r))
    //  financedRepay = (P − requiredDown) × (1 + r)  // <= L
    //  monthly = financedRepay / m
    const limit = Number(scoringLimit);
    const onePlusR = 1 + ratePct / 100;
    const reqDown = Math.max(0, Number(price) - limit / onePlusR);
    appliedDown = reqDown;
    const financedRepay = (Number(price) - appliedDown) * onePlusR; // konstruktsiya bo'yicha <= limit
    displayMonthly = months ? financedRepay / months : 0;
    grandTotal = appliedDown + financedRepay; // odatda min(P*(1+r), L) + (P - L/(1+r)) = P*(1+r) yoki L + down

    detailsBlock = (
      <div className="grid grid-cols-1 gap-3">
        <div className="h-px bg-neutral-200" />
        <StatRow label="To'liq hisob (tan narx + ustama)" value={`${fmt(repayFull)} so'm`} />
        <StatRow label="Scoring limiti (JAMI)" value={`${fmt(limit)} so'm`} />
        <StatRow label="Kerakli boshlang'ich (hisoblangan)" value={`${fmt(appliedDown)} so'm`} />
        <StatRow label="Bo'lib-bo'lib jami (hisoblangan)" value={`${fmt(financedRepay)} so'm`} />
      </div>
    );
  } else {
...
function runSanityTests() {
  // Test 0: DEFAULT_RATES match expected decimals
  ...
  // Test 1: Yearly-limit sanity (dynamic)
  const P1 = 17000000, L1 = 12000000, U1 = 0, m1 = 10, r1 = DEFAULT_RATES[10] / 100;
  const down1 = Math.max(0, P1 - Math.max(0, L1 - U1));
  const financed1 = P1 - down1;
  const monthly1 = (financed1 * (1 + r1)) / m1;
  console.assert(financed1 === 12000000, 'financed1 should be 12,000,000');
  console.assert(monthly1 > 0, 'monthly1 positive');

  // Test 2: Scoring-limit NEW logic
  const P2 = 14000000, m2 = 10, r2 = DEFAULT_RATES[10] / 100, L2 = 16000000;
  const D2 = Math.max(0, P2 - L2 / (1 + r2));
  const repay2 = (P2 - D2) * (1 + r2);
  console.assert(Math.abs(repay2 - L2) < 2, 'repay2 should equal limit when binding');
  console.assert(Math.abs(D2 - (P2 - L2 / (1 + r2))) < 2, 'down matches formula');

  // Test 3: Clamp/fmt
  console.assert(clamp(5,0,10)===5 && clamp(-1,0,10)===0 && clamp(11,0,10)===10, 'clamp ok');
  console.assert(fmt(1234567) === new Intl.NumberFormat('uz-UZ').format(1234567), 'fmt ok');
}
